...
function loadPresets() {
	if (presetsPromise === null) {
		presetsPromise = fetch('/extensions/ComfyUI-PromptBuilderKrea/presets.json?t=' + Date.now())
...
function buildPrompt(node) {
	...
	const cameraName = node.widgets.find(w => w.name === 'camera')?.value || '';
	...
	const backgroundValue = getPresetValue('background', backgroundName);
	const lightingValue = getPresetValue('lighting', lightingName);
	const styleValue = getPresetValue('style', styleName);
	const cameraValue = getPresetValue('camera', cameraName);

	return `${subjectValue}, ${poseValue}, ${outfitValue}, ${backgroundValue}, ${lightingValue}, ${styleValue}, ${cameraValue}.`;
}
...
app.registerExtension({
	name: 'PromptBuilderKreaNode.AutoUpdate',
	async nodeCreated(node) {
		if (node.comfyClass !== 'PromptBuilderKreaNode') {
			return;
		}
		...
		setTimeout(() => {
			...
			const observedWidgets = ['subject', 'pose', 'outfit', 'background', 'lighting', 'style', 'camera'];
